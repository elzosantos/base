                ...
                <div class="flex justify-between">
                    <span>{{$reply['created_at']}}</span>
                        ...
                        <form action="{{ route('replies.destroy', [$support->id, $reply['id']]) }}" method="post">
                            @csrf()
                            @method('DELETE')
                            <button type="submit" class="bg-orange-500 hover:bg-orange-400 text-white py-1 px-4 border-b-4 border-orange-700 hover:border-orange-500 rounded">Deletar</button>
                        </form>
